...
## 초록
모바일 엣지 컴퓨팅(Mobile Edge Computing, MEC) 환경의 계산 오프로딩은 사용자 체감 품질(Quality of Experience, QoE), 서비스 지연, 에너지 소비, 그리고 마감 시간 내 작업 완료 여부를 동시에 고려해야 하는 다목적 문제이다. 본 연구는 기존 QoE-Oriented Computation Offloading (QECO)에 부하 적응형 에너지 제어를 결합한 제안 변형 알고리즘 QECO-ADAPT를 정의하고, 이를 Deep Reinforcement Learning for Online Computation Offloading (DROO), 기존 QECO, 그리고 Tang&Wong-style Deep Q-Network (DQN) 기반 알고리즘과 동일한 공통 MEC 테스트베드에서 비교한다[1][2][6]. 공통 환경은 동일한 난수 시드, 동일한 작업 생성 파이프라인, 동일한 지연 제한, 동일한 episode 길이를 공유하도록 재구성되었다. 본 연구는 QECO-ADAPT의 효과를 두 축으로 평가한다. 첫째, 전체 episode 평균은 학습 초반 수렴 구간에서 누적되는 QoE 손실과 dropped-task count를 얼마나 줄이는지 보여주는 지표로 사용한다. 둘째, 마지막 10% episode 평균은 후반 안정 구간에서 기존 QECO의 성능을 얼마나 잘 따라가는지 확인하는 지표로 사용한다. 총 7개 시나리오(`users=10, 30, 50, 80`, `edges=1, 3, 10`)를 분석한 결과, QECO-ADAPT는 저부하 단일 edge 환경에서는 QECO 대비 명확한 우위를 보이지 않았으나, 사용자 수가 증가하거나 edge 수가 확대된 대부분의 환경에서는 전체 episode 평균 기준 QoE, Delay, Energy, Drop을 함께 개선하는 경향을 보였다. 반면 후반 안정 구간에서는 QECO 대비 큰 구조적 우위를 항상 보이기보다, 기존 QECO의 QoE-Delay-Drop 균형을 대체로 따라가면서 에너지 항을 완화하는 형태로 수렴했다. 이러한 결과는 QECO-ADAPT의 핵심 가치가 최종 안정 성능의 일방적 우월성보다는, 동적 MEC 환경에서 warm-up 손실을 줄이고 이후 QECO 수준의 안정 정책으로 수렴하는 부하 적응형 제어에 있음을 시사한다.
...
기존 구현은 서로 다른 환경 설정과 시간축을 사용하므로 직접 비교가 어렵다. 이에 본 연구에서는 공통 MEC 평가 환경을 구성하여 DROO, QECO, QECO-ADAPT, Tang&Wong-style DQN을 동일한 조건에서 비교한다[6]. 비교 차트에는 포함하지 않았지만, 좌표하강법(Coordinate Descent, CD)은 동일한 `user=10`, `edge=1`, `episode=400` 보조 실험에서 `trimmed median episode runtime`이 `14.3218s`로 측정되어, QECO의 `1.4273s`, QECO-ADAPT의 `1.4349s`보다 약 10배 길었다. 따라서 CD는 실사용 관점의 주 비교군에서 제외하였다.

![CD auxiliary runtime comparison](./experiment_results/comparisons/user_10_ep_400_edge_1__cd-qeco-qeco-adapt/comparison_runtime.png)

**보조그림 A1. CD, QECO, QECO-ADAPT의 보조 runtime 비교.** 동일한 `user=10`, `edge=1`, `episode=400` 조건에서 CD의 trimmed median episode runtime이 QECO 계열 대비 현저히 크게 나타남을 보여준다.
...
### 2.3 전체 평균과 후반 안정 구간 집계 방식
본 연구는 QECO-ADAPT의 성격을 분리해 보기 위해 두 가지 집계 축을 함께 사용한다. 첫째, `전체 episode 평균`은 400 episode 전체에서의 평균 성능을 나타내며, 초반 탐험과 수렴 구간에서 발생하는 QoE 손실, 지연 증가, 에너지 낭비, dropped-task count 누적까지 포함한다. 따라서 이는 QECO-ADAPT가 warm-up 구간에서 얼마나 빠르게 실용적인 정책 품질에 도달하는지를 해석하기 위한 주 지표이다. 둘째, `final 10% 평균`은 raw episode 시계열의 마지막 10% 구간 평균으로 정의하며, 수렴 이후 후반 안정 구간에서 기존 QECO의 성능을 얼마나 잘 따라가는지 확인하기 위한 보조 지표이다.

이때 `QoE`, `Delay`, `Energy`는 episode 내부에서 실제 도착한 task 집합에 대한 산술평균이며, `Drop`은 해당 episode에서 deadline 내 완료되지 못한 task의 총 개수이다. 따라서 `Drop`은 task 전체 대비 drop ratio가 아니라 episode 단위 dropped-task count로 해석해야 한다.

![Metric aggregation formulas](./experiment_results/formula_visualizations/metric_aggregation_formula.png)

**그림 1. 공통 MEC 평가에서 사용한 task-level QoE, episode-level metric, final 10% 집계식.** 본문에서는 전체 episode 평균을 초반 수렴성 해석의 주 지표로, final 10% 평균을 후반 안정 구간 확인 지표로 사용한다.

### 2.4 CD 제외 근거
CD는 품질 지표만 놓고 보면 `user=10`, `edge=1`, `episode=400` 환경에서 마지막 10% 평균 기준 `QoE=22.1509`, `Drop=49.4500` 수준으로 완전히 무의미한 기준선은 아니었다. 그러나 같은 환경에서 CD의 `trimmed median episode runtime`은 `14.3218s`였고, QECO와 QECO-ADAPT는 각각 `1.4273s`, `1.4349s`에 머물렀다. 즉 CD는 같은 공통 환경에서 약 10배 수준의 runtime overhead를 보였으며, 이러한 계산 비용은 반복 실험과 실제 online offloading 관점에서 부담이 크다. 이 차이는 보조그림 A1의 runtime 비교에서도 확인된다. 본 연구가 학습 기반 정책의 실사용 가능성을 함께 고려한다는 점에서, CD는 참고용 보조 결과로만 유지하고 주 비교표에서는 제외하였다.
...
## 5. 시나리오별 비교 결과
### 5.1 대표 시나리오의 후반 안정 구간 결과
아래 표는 네 개의 대표 시나리오에서 마지막 10% 평균 기준 결과를 정리한 것이다. 이 표는 최종 안정 구간에서 각 알고리즘이 어떤 성능으로 수렴하는지를 보여준다. 즉 QECO-ADAPT가 후반에 QECO를 완전히 대체하는지보다, QECO의 안정 성능을 얼마나 근접하게 따라가는지를 확인하기 위한 자료이다. S1은 저부하 단일 edge, S2는 중간 부하 단일 edge, S3는 edge 확장 효과, S4는 QECO-ADAPT의 고부하 단일 edge 개선 효과를 확인하기 위한 대표 조건이다.
...
### 5.2 비교 그래프 자료
표 5.1의 수치 결과를 보완하기 위해, 모든 주요 시나리오에 대한 전체 episode 평균 그래프, final 10% 평균 그래프, episode 기반 time-series 그래프를 함께 제시하였다. 아래 그림들은 하나의 통합 이미지가 아니라 각 시나리오별 원본 PNG 파일을 개별로 연결한 것이다. 따라서 GitHub 저장소에서도 각 그래프 파일이 독립적으로 관리되며, 본문에서 이미지를 클릭하면 원본 크기로 확인할 수 있다. 특히 왼쪽의 average 그래프는 QECO-ADAPT의 초반 수렴 이득을, final 그래프는 후반 안정 구간에서 QECO 성능을 추종하는 정도를 해석하는 데 사용한다.
...
**그림 5. S1 저부하 단일 edge 환경의 비교 그래프.** 왼쪽은 위에서부터 전체 episode 평균, final 10% 평균 지표, runtime 비교이며, 오른쪽은 episode 기반 smoothed time-series이다.
...
**그림 6. S2 중간 부하 단일 edge 환경의 비교 그래프.** 왼쪽은 위에서부터 전체 episode 평균, final 10% 평균 지표, runtime 비교이며, 오른쪽은 episode 기반 smoothed time-series이다.

#### S3: `user=30`, `edge=3`
<table>
  <tr>
    <td width="50%" align="center">
      <img src="./experiment_results/comparisons/user_30_ep_400_edge_3/comparison_averages.png" alt="S3 full episode average comparison"><br>
      <img src="./experiment_results/comparisons/user_30_ep_400_edge_3/comparison_finals.png" alt="S3 final 10 percent comparison"><br>
      <img src="./experiment_results/comparisons/user_30_ep_400_edge_3/comparison_runtime.png" alt="S3 trimmed median runtime comparison">
    </td>
    <td width="50%" align="center">
      <img src="./experiment_results/comparisons/user_30_ep_400_edge_3/comparison_timeseries_smoothed.png" alt="S3 smoothed episode time-series comparison">
    </td>
  </tr>
</table>

**그림 7. S3 중간 부하 다중 edge 환경의 비교 그래프.** 왼쪽은 위에서부터 전체 episode 평균, final 10% 평균 지표, runtime 비교이며, 오른쪽은 episode 기반 smoothed time-series이다.
...
**그림 8. S4 고부하 단일 edge 환경의 비교 그래프.** 왼쪽은 위에서부터 전체 episode 평균, final 10% 평균 지표, runtime 비교이며, 오른쪽은 episode 기반 smoothed time-series이다.

#### S5: `user=50`, `edge=10`
<table>
  <tr>
    <td width="50%" align="center">
      <img src="./experiment_results/comparisons/user_50_ep_400_edge_10/comparison_averages.png" alt="S5 full episode average comparison"><br>
      <img src="./experiment_results/comparisons/user_50_ep_400_edge_10/comparison_finals.png" alt="S5 final 10 percent comparison"><br>
      <img src="./experiment_results/comparisons/user_50_ep_400_edge_10/comparison_runtime.png" alt="S5 trimmed median runtime comparison">
    </td>
    <td width="50%" align="center">
      <img src="./experiment_results/comparisons/user_50_ep_400_edge_10/comparison_timeseries_smoothed.png" alt="S5 smoothed episode time-series comparison">
    </td>
  </tr>
</table>

**그림 9. S5 고부하 다중 edge 환경의 비교 그래프.** 왼쪽은 위에서부터 전체 episode 평균, final 10% 평균 지표, runtime 비교이며, 오른쪽은 episode 기반 smoothed time-series이다.
...
**그림 10. S6 매우 높은 부하 단일 edge 환경의 비교 그래프.** 왼쪽은 위에서부터 전체 episode 평균, final 10% 평균 지표, runtime 비교이며, 오른쪽은 episode 기반 smoothed time-series이다.

#### S7: `user=80`, `edge=3`
<table>
  <tr>
    <td width="50%" align="center">
      <img src="./experiment_results/comparisons/user_80_ep_400_edge_3/comparison_averages.png" alt="S7 full episode average comparison"><br>
      <img src="./experiment_results/comparisons/user_80_ep_400_edge_3/comparison_finals.png" alt="S7 final 10 percent comparison"><br>
      <img src="./experiment_results/comparisons/user_80_ep_400_edge_3/comparison_runtime.png" alt="S7 trimmed median runtime comparison">
    </td>
    <td width="50%" align="center">
      <img src="./experiment_results/comparisons/user_80_ep_400_edge_3/comparison_timeseries_smoothed.png" alt="S7 smoothed episode time-series comparison">
    </td>
  </tr>
</table>

**그림 11. S7 매우 높은 부하 다중 edge 환경의 비교 그래프.** 왼쪽은 위에서부터 전체 episode 평균, final 10% 평균 지표, runtime 비교이며, 오른쪽은 episode 기반 smoothed time-series이다.

모든 run은 현재 공통 환경에서 episode 수를 동일하게 맞추었기 때문에, time-series는 별도의 1000-point interpolation이 아니라 raw episode 기준의 수렴 경향을 보여준다. 따라서 그림 5부터 그림 11까지의 오른쪽 그래프는 QECO-ADAPT가 초기 수렴 구간과 중·고부하 조건에서 어떤 방향으로 변하는지 확인하기 위한 보조 자료로 해석한다. 왼쪽의 average, final, runtime 그래프는 각각 전체 학습 구간 평균, 최종 안정 구간 평균, 계산 비용을 함께 검증하기 위한 자료로 사용한다.

### 5.3 후반 안정 구간 시나리오 요약
각 시나리오에서 final 10% 평균 기준으로 가장 두드러진 결과를 요약하면 다음과 같다. 이 표는 최종 수렴 이후의 안정 성능을 보여주므로, 전체 학습 구간에서의 초반 수렴 이득은 다음 절의 전체 episode 평균 비교와 함께 해석해야 한다.
...
QECO-ADAPT의 효과는 전체 episode 평균과 final 10% 평균에서 다르게 읽힌다. 전체 episode 평균은 초반 수렴 구간의 손실까지 포함하므로, QECO-ADAPT가 학습 초반부터 더 빨리 실용적 정책 품질에 도달했는지를 보여준다. 반면 final 10% 평균은 후반 안정 구간에서 QECO의 성능을 얼마나 잘 따라가는지를 보여준다. 아래 표에서 `QoE`는 양수일수록 좋고, `Delay`, `Energy`, `Drop`은 음수일수록 좋다.

### 6.1 전체 episode 평균 기준 변화량
전체 episode 평균에서는 S1을 제외한 모든 시나리오에서 QECO-ADAPT가 QECO보다 QoE를 높이고 Delay와 Drop을 낮췄다. 이는 QECO-ADAPT의 주된 이득이 최종 안정 성능의 일방적 우위가 아니라, 학습 초반 수렴 손실을 줄이는 데 있음을 보여준다.

| 시나리오 | ΔQoE | ΔDelay | ΔEnergy | ΔDrop | 해석 |
| --- | ---: | ---: | ---: | ---: | --- |
| S1: 10/1 | -2.1422 | +0.2348 | -1.0399 | +13.0650 | 저부하에서는 adaptive penalty가 과도하게 작동 |
| S2: 30/1 | +1.2513 | -0.1039 | -1.0290 | -21.3000 | 단일 edge dense 조건에서 초반 수렴 이득이 나타나기 시작 |
| S3: 30/3 | +0.0193 | -0.0077 | +0.0035 | -0.3325 | 다중 edge에서는 QECO와 거의 동일한 평균 성능을 유지 |
| S4: 50/1 | +2.6892 | -0.1565 | -2.4354 | -79.0525 | 고부하 단일 edge에서 초반 손실 감소가 가장 뚜렷 |
| S5: 50/10 | +0.1907 | -0.0414 | -0.0736 | -4.5250 | edge가 충분한 환경에서도 평균 성능을 소폭 개선 |
| S6: 80/1 | +3.3372 | -0.1473 | -3.6481 | -154.8800 | 극고부하 단일 edge에서 QoE와 Drop 누적 손실을 크게 줄임 |
| S7: 80/3 | +0.2943 | -0.0318 | -0.2179 | -12.5925 | 고부하 다중 edge에서도 안정적인 평균 개선 |

### 6.2 후반 안정 구간 기준 변화량
final 10% 평균에서는 QECO-ADAPT가 대부분의 중·고부하 시나리오에서 QECO를 따라가거나 소폭 개선하지만, 전체 평균만큼 큰 차이를 보이지 않는다. 특히 S2에서는 전체 평균 기준 이득이 분명한 반면, 후반 안정 구간에서는 QECO와 거의 같은 수준으로 수렴한다. 이 차이는 QECO-ADAPT의 핵심 효과가 후반 성능 폭증이 아니라 초반 warm-up 손실 감소에 있음을 뒷받침한다.

| 시나리오 | ΔQoE | ΔDelay | ΔEnergy | ΔDrop | 해석 |
| --- | ---: | ---: | ---: | ---: | --- |
| S1: 10/1 | -1.7895 | +0.3271 | -0.9622 | +10.9500 | 저부하 후반 구간에서도 QECO가 더 안정적 |
| S2: 30/1 | -0.0709 | -0.0129 | -0.3169 | +4.8500 | 후반에는 QECO와 거의 동일한 수준으로 수렴 |
| S3: 30/3 | +0.2073 | -0.0206 | +0.1070 | -4.1750 | edge 확장 시 QECO를 근접 추종하며 Drop 개선 |
| S4: 50/1 | +0.2907 | -0.0367 | -0.3488 | -11.3750 | 고부하 단일 edge에서 후반 성능도 QECO보다 소폭 우세 |
| S5: 50/10 | +0.4442 | -0.0852 | -0.1561 | -12.4250 | 다중 edge 고부하에서도 안정 구간 개선 유지 |
| S6: 80/1 | +0.2094 | -0.0344 | -0.6711 | -6.6750 | 극고부하 후반에는 QECO 추종과 에너지 절감이 동시에 나타남 |
| S7: 80/3 | +0.3381 | -0.0507 | -0.2006 | -15.6750 | 고부하 다중 edge에서 QECO 대비 가장 안정적 |

두 표를 함께 보면 QECO-ADAPT의 성격이 명확해진다. 전체 episode 평균에서는 초반 수렴 과정에서 QoE와 Drop 손실을 줄이는 효과가 나타나고, final 10% 평균에서는 QECO의 안정 성능을 크게 벗어나지 않고 따라가며 일부 dense 조건에서 소폭 개선한다. 따라서 QECO-ADAPT는 QECO의 후반 수렴 성능을 대체하기 위한 알고리즘이라기보다, QECO 수준의 후반 안정성을 유지하면서 초기 학습 구간의 warm-up 손실을 줄이는 변형으로 해석하는 것이 적절하다.
...
### 7.3 QECO-ADAPT의 위치
현재 QECO-ADAPT는 모든 환경에서 QECO를 일괄적으로 대체하지는 않는다. 저부하인 S1에서는 전체 평균과 후반 안정 구간 모두에서 QECO가 더 자연스러운 QoE-Drop 균형을 보인다. 그러나 부하가 증가하면 해석이 달라진다. 전체 episode 평균에서는 S2부터 QECO-ADAPT가 QoE, Delay, Drop을 개선하고, 단일 edge dense 조건에서는 Energy도 함께 줄이며 초반 수렴 과정에서 발생하는 누적 손실을 낮춘다. 반면 final 10% 평균에서는 QECO-ADAPT가 QECO를 크게 앞서기보다 QECO의 안정 구간 성능을 따라가고, S4-S7 같은 dense 조건에서는 소폭 개선한다. 즉 QECO-ADAPT의 핵심 가치는 "후반 안정 성능의 일방적 우월성"이 아니라 "초반 수렴 손실을 줄이면서 후반에는 QECO 수준의 안정 정책을 유지하는 적응성"에 있다.
...
이 결과는 runtime 관찰이 단일 알고리즘의 절대적 우열보다 환경 조건에 민감하다는 점을 보여준다. 특히 QECO-ADAPT는 전체 평균 기준으로 중·고부하 조건에서 QoE, Delay, Drop 개선을 보였지만, runtime은 QECO보다 항상 낮지는 않았다. 따라서 QECO-ADAPT의 장점은 계산 시간 자체의 단축이라기보다, QECO와 같은 실행 시간 범위 안에서 초반 수렴 손실을 줄이는 부하 적응형 에너지 제어를 추가한다는 점에 있다.

### 7.6 QECO-ADAPT의 초반 수렴 이점과 MEC적 의미
실험 곡선과 전체 episode 평균을 함께 보면 QECO-ADAPT는 일부 시나리오에서 QECO보다 초반 수렴 구간의 손실을 더 빠르게 줄이는 경향을 보였다. 이 특성은 MEC 환경에서 단순 학습 속도 이상의 의미를 가진다.

우리 결과만 보더라도 이 해석에는 정량적 근거가 있다. 단일 edge 기준으로 `user=30`, `user=50`, `user=80` 조건에서는 QECO-ADAPT가 QECO보다 전체 episode 평균 QoE를 각각 `+1.2513`, `+2.6892`, `+3.3372` 높였고, 평균 Drop도 각각 `21.3000`, `79.0525`, `154.8800`만큼 낮췄다. 반면 같은 조건의 final 10% 평균에서는 S2가 QECO와 거의 같은 수준으로 수렴하고, S4와 S6은 QECO보다 소폭 개선하는 데 그친다. 즉 QECO-ADAPT의 이득은 후반 성능이 폭발적으로 높아지는 데서 나오기보다, 초반 학습 구간에서 더 이른 시점부터 실용적인 정책 품질에 도달해 전체 평균 손실을 줄이는 데서 나온다. 다만 현재의 초반 수렴 해석은 별도의 convergence-time metric이 아니라 episode 전체 평균과 final 평균의 차이에서 도출한 간접 해석이므로, 후속 실험에서는 목표 QoE 도달 episode, Drop 안정화 episode와 같은 직접 수렴 지표를 추가할 필요가 있다. 반대로 `user=10, edge=1` 같은 저부하 환경에서는 이러한 이점이 나타나지 않았으므로, 초반 수렴의 장점 역시 환경 의존적으로 해석해야 한다.
...
넷째, QECO 자체도 dynamic and uncertain mobile environment를 다루기 위한 QoE-oriented DRL 알고리즘으로 제안되었다[2]. 이러한 맥락에서 QECO-ADAPT의 빠른 초기 안정화는 단순한 학습 효율 향상을 넘어, deadline-sensitive MEC 서비스에서 초반 queue 누적과 task drop의 악화를 줄일 수 있는 잠재적 장점으로 연결된다. 동시에 final 10% 평균에서 QECO-ADAPT가 대체로 QECO를 추종한다는 점은, 제안 변형이 QECO의 후반 안정 정책을 무너뜨리지 않고 초기 warm-up 구간을 보완하는 방향으로 작동함을 의미한다. 본 연구는 아직 이 점을 다중 seed와 적응 시간 지표로 정량화하진 않았으나, 현재 결과는 QECO-ADAPT의 가치를 초기 수렴 성능과 후반 안정 추종성의 결합 관점에서 평가할 필요가 있음을 시사한다.
...
- QECO-ADAPT는 저부하 환경에서는 이득이 약하지만, 사용자 수 증가 또는 edge 확장 환경에서는 전체 episode 평균 기준 초반 수렴 손실을 줄인다.
- QECO-ADAPT는 후반 안정 구간에서 QECO를 일방적으로 압도하기보다, QECO의 QoE-Delay-Drop 균형을 대체로 따라가며 일부 dense 조건에서 소폭 개선한다.
- Tang&Wong-style DQN은 특히 다중 edge 환경에서 가장 강한 QoE와 Drop 성능을 보였다.

따라서 QECO-ADAPT는 단순히 QECO의 대체 모델이 아니라, QECO의 후반 안정 정책을 크게 훼손하지 않으면서 학습 초반의 warm-up 손실을 줄이는 환경 의존적 적응형 에너지 제어 메커니즘으로 해석하는 것이 적절하다. 이 관점에서 QECO-ADAPT의 핵심 기여는 "최종 구간에서 항상 더 높은 성능"이 아니라 "동적 MEC 환경에서 더 빠르게 실용적 정책 수준에 접근하고, 이후에는 QECO의 안정 성능을 따라가는 것"이다. 향후에는 `load_scale_base` 민감도 분석, `base_profile` 및 `user_activity` 범위 조정, 다중 seed 반복 실험, 그리고 초기 수렴 속도 자체를 비교하는 적응 시간 지표를 추가하여 QECO-ADAPT의 유효 구간을 더 엄밀하게 규정할 필요가 있다.
