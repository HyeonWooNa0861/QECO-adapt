...
다만 이 조정은 원래 QECO reward에 대한 policy-invariant reward shaping이 아니다. 부하가 증가할수록 $w_E$가 커지면 delay와 energy 사이의 상대 가중치가 달라지고, 그 결과 동일한 action 집합에서도 선호되는 action 순위가 바뀔 수 있다. 따라서 QECO-ADAPT에서 학습된 정책은 기존 QECO의 원래 QoE objective를 그대로 최적화한 정책이라기보다, dense 환경에서 energy pressure와 dropped-task 누적을 줄이도록 재가중한 load-adaptive surrogate reward에 대한 정책으로 해석해야 한다. 본 분석에서 QoE, Delay, Energy, Dropped tasks를 별도 지표로 함께 보고하는 이유도, 수정된 reward가 실제 평가 지표에서 어떤 trade-off를 만드는지 확인하기 위해서이다.
...
종합하면 QECO-ADAPT는 QECO를 모든 환경에서 대체하는 범용 알고리즘이 아니다. 오히려 저부하에서는 기존 QECO가 더 안정적이며, QECO-ADAPT는 dense MEC 환경에서 초반 수렴 손실을 줄이고 후반에는 QECO 수준의 안정 정책을 따라가는 보완형 알고리즘으로 정의하는 것이 타당하다. 또한 QECO-ADAPT는 기존 QECO objective의 무손실 개선이 아니라 reward landscape를 부하 적응적으로 재가중하는 방식이므로, 원래 QoE 최적 정책의 보존을 이론적으로 보장하지 않는다. 따라서 본 결과는 수정 reward에 대한 학습 정책이 공통 평가 지표에서 QoE, Delay, Energy, Dropped tasks의 trade-off를 어떻게 바꾸는지 보여주는 경험적 검증으로 해석해야 한다. 향후에는 다중 seed 반복 실험, 목표 QoE 도달 episode, dropped-task 안정화 episode와 같은 직접적인 수렴 지표를 추가하여 QECO-ADAPT의 warm-up 손실 감소 효과를 더 엄밀하게 검증할 필요가 있다.
...
[7] L. Sun, R. Liang, L. Wan, K. Liu, Z. Ning, and J. Wang, "Online Partial Computation Offloading Optimization in Wireless Powered Mobile Edge Computing Network," IEEE Transactions on Cognitive Communications and Networking, vol. 12, pp. 1481-1495, 2026, doi: 10.1109/TCCN.2025.3612741. 인용 용도: partial offloading의 유연성과 그에 따른 online optimization/resource scheduling 복잡성 배경. 원문 근거: "partial offloading decisions can be flexibly adjusted"; "brings challenges to the optimization." 부가 설명: binary offloading은 task 전체를 local에서 처리하거나 edge/AP로 전부 offload하는 선택이지만, partial offloading은 task data를 local computing 부분과 edge computing 부분으로 나누고 offloading ratio를 연속값 또는 후보 비율로 결정한다. 따라서 channel state, harvested energy, WPT duration, transmission time, AP computation resource, queue backlog에 따라 local 처리량과 offloaded 처리량을 함께 조절할 수 있다. 다만 가능한 offloading ratio 후보가 늘어나고 WPT 시간/전송 시간/자원 할당까지 결합되므로, 단순 binary action보다 action/resource scheduling 공간이 커져 online 최적화가 어려워진다. 본 연구의 QECO-ADAPT는 이러한 partial offloading의 표현력 자체를 도입한 것은 아니며, 기존 QECO의 action structure를 유지한 채 effective load에 따라 energy penalty와 offloading 보수화 강도를 조절하는 lightweight adaptation으로 차별화된다.
